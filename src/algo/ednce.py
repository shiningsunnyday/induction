...
def update_graph(g, anno, best_ism, best_clique, grammar, index=-1):
    ...
    for c in best_clique:
        ...
        anno[new_n] = EDNCENode(new_n, attrs={'rule': len(grammar.rules)-1 if index == -1 else index})
...
def compress(g, grammar, anno):
    ...
    while changed:
        ...
        if best_clique is not None:
            ...
            g, _ = update_graph(g, anno, best_ism, best_clique, grammar, index=best_rule_idx)
    return g, anno
